                    ...
                    case 'match':
                        ...
                        switch(command[1]) {
                            ...
                            case 'team2':
                                ...
                                break;
                            case 'swap':
                                message.reply(await cmds.matchSwap(command[2], user.id))
                                break;
                            case 'delete':
                            ...
                            case 'start':
                                ...
                                if (match !== undefined) {
                                    if (match.ready) {
                                        ...
                                        //создаем фильтры
                                        var filter1 = (reaction, user) => emojiNumbers.includes(reaction.emoji.name) && user.id === match.captain1;
                                        var filter2 = (reaction, user) => emojiNumbers.includes(reaction.emoji.name) && user.id === match.captain2;
                                        //ждем голос
                                        var vote;
                                        ...
                                        for (let i = 0; i < 8; i++) {
                                            ...
                                            let mapNumber = emojiNumbers.indexOf(vote.first().emoji.name);//определяем выбранную цифру
                                            ...
                                            if (flag) {
                                                await vote.first().remove(match.captain1);//удаляем реакцию капитана
                                            } else {
                                                await vote.first().remove(match.captain2);//удаляем реакцию капитана
                                            }
                                            poolStr = "";
                                        ...
                                        await message.channel.send('MAP VOTE COMPLETED```JSON\n'+JSON.stringify(match, null, 2)+'```');

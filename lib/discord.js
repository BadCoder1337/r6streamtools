...
function Ops() {
    this.atk = ['ash', 'blackbeard', 'blitz', 'buck', 'capitao', 'dokkaebi', 'finka', 'fuze', 'glaz', 'hibana', 'iq', 'jackal', 'lion', 'montagne', 'sledge', 'thatcher', 'thermite', 'twitch', 'ying', 'zofia'];
    this.def = ['bandit', 'castle', 'caveira', 'doc', 'echo', 'ela', 'frost', 'jager', 'kapkan', 'lesion', 'mira', 'mute', 'pulse', 'rook', 'smoke', 'tachanka', 'valkyrie', 'vigil'];
}
function EmojiPoll() {
                                    ...
                                    if (match.ready) {
                                        ...
                                        if (match.banOps) {
                                            ...
                                            for (let i = 0; i < mapAmount; i++) {
                                                ...
                                                confirm = await poll.edit(`<@${match.captain1}>,<@${match.captain2}>, бан оперативников для карты №${i+1} ${match.pool.name[match.pool.id.indexOf(match.votes.slice(9-mapAmount)[i])]} закончен! Удаленные оперативники: \`${match.votesOps.join(', ')}\`. <@${match.creatorID}>, продолжить?`);
                                                await confirm.react('🆗');
                                                await confirm.react('🆑');
                                                var filter = (reaction, user) => (reaction.emoji.name === '🆗' || reaction.emoji.name === '🆑') && user.id === match.creatorID
                                                let res = await confirm.awaitReactions(filter, { maxEmojis: 1 });

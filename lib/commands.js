...
function Pool() {
    this.id = ['bartlett', 'chalet', 'favela', 'house', 'kafe', 'kanal', 'plane', 'skyscraper', 'themepark', 'tower', 'yacht'];
    this.name = ['Ун. Бартлет', 'Шале', 'Фавела', 'Дом', 'Кафе Достоевский', 'Канал', 'Борт №1', 'Небоскреб', 'Лунопарк', 'Башня', 'Яхта'];
}
// function Ops() {
    ...
    if (id && code && title && captain) {
        ...
        if (item !== undefined) {
            ...
            if (item.captain1 && item.captain2) {
                item.ready = true;
                ee.emit('match', { match: item, type: 'reload' });
    ...
    if (match !== undefined) {
        ...
        if (match.ready) {
            ...
            for (let i = 0; i < match.votes.length; i++) {
                if ((match.matchType == 'bo3' && [match.votes.length-1,3,2].includes(i)) || (match.matchType == 'bo5' && [match.votes.length-1,5,4,3,2].includes(i)) || (match.matchType == 'bo1' && i == (match.votes.length-1))) {
                    poolStr += `${(ii).toString(16).toUpperCase()}. **${match.pool.name[match.pool.id.indexOf(match.votes[i])]}**  (сторону выбирает <@${match['captain'+(ii%2+1)]}>)\n`;
                    ii++;
                }
            }
            await poll.delete();
